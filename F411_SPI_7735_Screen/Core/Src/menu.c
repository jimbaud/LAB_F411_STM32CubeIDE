...
void drawMenu(void) {
	...
	for (int i = startItem; i < startItem + visibleItems; i++) {
		...
		ST7735_FillRectangle(RECT_X, RECT_Y1 + (i - startItem) * (RECT_HEIGHT + RECT_SPACING), RECT_WIDTH, RECT_HEIGHT, rectColor);
...
void drawBarGraph(float value) {
	...
	if (barWidth > 0) {
		if (barWidth <= partWidth) {
			// La partie verte
			ST7735_FillRectangle(BAR_X, BAR_Y, barWidth, BAR_HEIGHT, greenColor);
		} else if (barWidth <= 2 * partWidth) {
			// La partie verte et orange
			ST7735_FillRectangle(BAR_X, BAR_Y, partWidth, BAR_HEIGHT, greenColor);
			ST7735_FillRectangle(BAR_X + partWidth, BAR_Y, barWidth - partWidth, BAR_HEIGHT, orangeColor);
		} else {
			// La partie verte, orange et rouge
			ST7735_FillRectangle(BAR_X, BAR_Y, partWidth, BAR_HEIGHT, greenColor);
			ST7735_FillRectangle(BAR_X + partWidth, BAR_Y, partWidth, BAR_HEIGHT, orangeColor);
			ST7735_FillRectangle(BAR_X + 2 * partWidth, BAR_Y, barWidth - 2 * partWidth, BAR_HEIGHT, redColor);
		}
	}

	// Afficher la valeur numérique en dessous du graphique
	char valueText[10];
	sprintf(valueText, "%.1f%%", value);
	int textWidth = strlen(valueText) * 6; // Largeur approximative du texte en pixels
	int textX = BAR_X + BAR_WIDTH / 2 - textWidth / 2;
	int textY = BAR_Y + BAR_HEIGHT + 5; // 5 pixels en dessous de la barre
...
uint32_t ReadAndConvertADC(void) {
	...
	uint32_t convertedValue = (adcValue * 100) / ADC_RESOLUTION;
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
	if (GPIO_Pin == Push_Btn_Pin) { // Remplacez GPIO_PIN_0 par la broche que vous utilisez
